...
# Как это работает?
# 1. k - это указатель на позицию, куда будет записан следующий уникальный элемент.
# 2. Начинаем с k = 1, так как первый элемент всегда уникален.
# 3. Проходим по массиву начиная со второго элемента.
# 4. Если текущий элемент отличается от предыдущего, значит это новый уникальный элемент.
# 5. Записываем его на позицию k и увеличиваем k на 1.
# 6. В конце k содержит количество уникальных элементов.
# 7. Данный вариант имеет сложность O(n) по времени и O(1) по дополнительной памяти,
# что соответствует требованиям задачи.
